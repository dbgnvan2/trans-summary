...
7. **Producer/consumer format contract (P2/P14).** For every LLM stage, does the *saver's* on-disk format exactly match what its *parser/validator/counter* re-reads (headings, blank lines between header & quote, `###` vs `> **`, verbatim vs paraphrase grounding)? Any parser/validator that can yield 0/zero from a non-empty artifact must call `warn_if_empty_parse` (`transcript_utils.py`) so "format drift" is loud, never a clean-looking empty pass. Round-trip test: save → re-parse → assert count preserved.
