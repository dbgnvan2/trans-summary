import pytest
from transcript_utils import markdown_to_html, normalize_text, strip_yaml_frontmatter, extract_section, parse_filename_metadata
...
def test_markdown_to_html_italic():
    assert markdown_to_html("This is *italic* text.") == "<p>This is <em>italic</em> text.</p>"

def test_markdown_to_html_paragraphs():
    assert markdown_to_html("Paragraph 1\n\nParagraph 2") == "<p>Paragraph 1</p>\n<p>Paragraph 2</p>"
...
def test_normalize_text_aggressive():
    assert normalize_text("This is a **Speaker:** test.", aggressive=True) == "this is a test"
    assert normalize_text("This has punctuation,!-.", aggressive=True) == "this has punctuation"
...
def test_parse_filename_metadata_with_hyphens():
    metadata = parse_filename_metadata("A Title - With - Hyphens - Jane Doe - 2025-01-01.md")
...
def test_parse_filename_metadata_formatted():
    metadata = parse_filename_metadata("Some Title - Some Presenter - 2023-03-03 - formatted.md")
