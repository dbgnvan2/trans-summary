...
- **MD-collection Bundle (DOC/PDF)**: Package a run's Markdown artifacts into a single **PDF** and/or **DOCX** — **Abstract first (on its own page)**, then Transcript (Formatted or YAML), Topics, Emphasis, Bowen References. Available two ways — a **"Bundle (DOC/PDF)" pipeline stage** (all sections, during a run) and a post-run **"Create Bundle…" dialog** to pick sections + format. PDF renders via WeasyPrint (reduced heading sizes); DOCX via pandoc (`--reference-doc`). Sections/order/page-breaks live in `config.BUNDLE_SECTIONS`. CLI: `python transcript_bundle.py "<base name>" --format pdf|docx|both`. Spec: `docs/spec_bundle_export_2026-07-16.md`.
...
- **Abstract length**: Abstracts are capped **under 250 words** — the target (`config.abstract_target_word_count`, ~3% of the transcript) is floored at `ABSTRACT_MIN_WORDS` and capped at `ABSTRACT_MAX_WORDS` (230); validation flags any abstract ≥ 250 words.
