...
def format_review_checklist(
    ...
    if coverage_items:
        ...
        for item in coverage_items:
            ...
            source_display = item.source_text[:80] + "..." if len(item.source_text) > 80 else item.source_text
    ...
    if proportion_issues:
        ...
        for issue in proportion_issues:
            ...
            lines.append(f"  Expected: ~{issue['expected']} words, Actual: {issue['actual']} words")
            lines.append("")
...
def validate_structural(summary: str, target_word_count: int) -> dict:
    ...
    evaluative_terms = ['important', 'valuable', 'insightful', 'excellent', 'crucial', 'significant']
