...
def generate_coverage_items(summary_input) -> list[CoverageItem]:
    ...
    # Opening section
    if (
        summary_input.opening.stated_purpose
        and "not explicitly stated" not in summary_input.opening.stated_purpose.lower()
    ):
        items.append(
            CoverageItem(
                category="opening",
    ...
    # Closing section
    if (
        summary_input.closing.conclusion
        and "no explicit conclusion" not in summary_input.closing.conclusion.lower()
    ):
        items.append(
            CoverageItem(
                category="closing",
...
def segment_summary(summary: str) -> dict:
    ...
    if paragraphs and len(paragraphs[0].split()) < 5:
        ...
        if not paragraphs[0].endswith(".") or paragraphs[0].lower() in [
            "summary",
            "abstract",
            "introduction",
        ]:
...
def check_keyword_coverage(
    text: str, item: CoverageItem, threshold: int = 2
) -> tuple[bool, str]:
...
def check_proportionality(summary: str, summary_input, tolerance: float = 0.3) -> dict:
    """
    ...
    # Overall assessment
    results["proportionality_ok"] = results["total_deviation"] <= tolerance and all(
        s["within_tolerance"] for s in results["sections"]
    )
...
def validate_summary_coverage(
    ...
    return {
        ...
        if needs_review or proportion_issues
        else None,
...
def format_review_checklist(
    coverage_items: list[CoverageItem], proportion_issues: list[dict]
) -> str:
