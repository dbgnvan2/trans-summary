...
from formatting_pipeline import load_prompt
...
def _generate_validation_response(prompt: str, model: str, temperature: float, logger, min_length: int = 50, system: Optional[list] = None) -> str:
...
def validate_emphasis_items(formatted_file_path: Path, extracts_summary_path: Path, logger):
    ...
    for label, quote in quotes:
        ...
        _, _, ratio = find_text_in_content(quote_core, formatted_content, aggressive_normalization=True)
...
def validate_headers(formatted_filename: str, model: str = config.AUX_MODEL, logger=None) -> bool:
    ...
    try:
        ...
        full_prompt = prompt_template.replace("{{batch_content}}", "(See transcript in system message)")
        ...
        response = _generate_validation_response(
            full_prompt, model, config.TEMP_STRICT, logger, min_length=100, system=system_message
        )
...
def validate_abstract_legacy(base_name: str, model: str, target_score: float, max_iterations: int, auto_continue: bool, logger) -> bool:
    ...
    try:
        ...
        for i in range(max_iterations + 1):
            ...
            validation_output = _generate_validation_response(
                prompt, model, config.TEMP_BALANCED, logger, min_length=config.MIN_ABSTRACT_VALIDATION_CHARS, system=system_message)
