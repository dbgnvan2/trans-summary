...
class CostEstimator:
    ...
    def _calculate_cost(
        ...
        if is_cache_write:
            ...
            cost_cache_write = (input_tokens / 1_000_000) * prices.get(
            ...
            cost_cache_read = (cached_tokens / 1_000_000) * prices["cache_read"]
        else:
    ...
    def estimate_formatting(self):
        ...
        prompt = (config.PROMPTS_DIR / config.PROMPT_FORMATTING_FILENAME).read_text()
    ...
    def estimate_summarization_steps(self):
        ...
        prompt = (config.PROMPTS_DIR / config.PROMPT_EXTRACTS_FILENAME).read_text()
        prompt_tokens = estimate_token_count(prompt)
        output_tokens = int(self.transcript_tokens * config.TARGET_EXTRACTS_PERCENT)
        self._calculate_cost(
            "2a. Key Items",
            ...
            output_tokens,
            is_cached_read=True,
            cached_tokens=self.transcript_tokens,
        )

        # 2b. Scored Emphasis
        prompt = (
            config.PROMPTS_DIR / config.PROMPT_EMPHASIS_SCORING_FILENAME
        ).read_text()
        prompt_tokens = estimate_token_count(prompt)
    ...
    def estimate_structured_steps(self):
        ...
        prompt = (
            config.PROMPTS_DIR / config.PROMPT_STRUCTURED_SUMMARY_FILENAME
        ).read_text()
        prompt_tokens = estimate_token_count(prompt) + json_input_tokens
        output_tokens = estimate_token_count(
            " " * config.DEFAULT_SUMMARY_WORD_COUNT * 5
        )  # 5 chars/word
        self._calculate_cost(
            "3a. Structured Summary",
            config.AUX_MODEL,
        ...
        prompt = (
            config.PROMPTS_DIR / config.PROMPT_STRUCTURED_ABSTRACT_FILENAME
        ).read_text()
        prompt_tokens = estimate_token_count(prompt) + json_input_tokens
        output_tokens = estimate_token_count(" " * config.ABSTRACT_MIN_WORDS * 5)
        self._calculate_cost(
            "3b. Structured Abstract",
            config.AUX_MODEL,
            prompt_tokens,
    ...
    def estimate_validation_steps(self):
        ...
        num_sections = (self.transcript_tokens * config.CHARS_PER_TOKEN) / (200 * 5)
        num_batches = math.ceil(num_sections / BATCH_SIZE)

        self._log(f"   (Estimating Header Validation as {num_batches} batches)")
    ...
    def print_report(self):
        ...
        self._log("model responses, prompt template changes, and API pricing updates.")
        self._log("=" * 100)
